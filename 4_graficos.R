...
df_detalhado <- df_raw %>%
  mutate(
    Grupo = case_when(
      ...
      str_detect(Variavel, "^VG_") ~ "Climate (Vegetative phase)",
      str_detect(Variavel, "^RP_") ~ "Climate (Reproductive phase)",
      str_detect(Variavel, "^FG_") ~ "Climate (Filling Grain phase)",
  ...
  mutate(
    Grupo = factor(Grupo, levels = c(
      "Soil", 
      "Climate (Vegetative phase)", 
      "Climate (Reproductive phase)", 
      "Climate (Filling Grain phase)", 
...
cores_oficiais <- c(
  ...
  "Climate (Vegetative phase)" = "#01665e",    # Verde escuro
  "Climate (Reproductive phase)" = "#8da0cb",  # Verde médio
  "Climate (Filling Grain phase)" = "#c7eae5"   , # Verde água clarinho
...
conferencia <- df_calculado %>%
  group_by(Grupo) %>%
  summarise(Soma_da_phase = sum(Porcentagem_Natural), .groups = 'drop')

print(conferencia)
message("Soma de tudo no modelo (Apenas Clima): ", round(sum(conferencia$Soma_da_phase), 1), "%")
...
grafico_natural <- ggplot(df_calculado, aes(x = Porcentagem_Natural, 
  ...
  scale_fill_manual(values = cores_phases) +
